...
func runMonkeyBusiness(in []*monkey, rounds int, simplify func(in int) int) {
	for i := 0; i < rounds; i++ {
		for _, m := range in {
...
func part1(in []*monkey) {
	runMonkeyBusiness(in, 20, func(in int) int { return in / 3 })
}

func part2(in []*monkey) {
	runMonkeyBusiness(in, 10000, func(in int) int { return in % 9699690 })
}
